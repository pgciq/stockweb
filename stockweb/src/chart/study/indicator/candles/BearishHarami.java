...
//		double SombraInf2 = candle.getCandleSize(1 ,2 ,2);
		...
		boolean TerceiroCandleAlta = (candle.Open(2) < candle.Close(2)) && (CorpoCandle3 > CorpoCandle2) && ( CorpoCandle3 > (SombraSup3+SombraInf3) );

		boolean SegundoCandle = ((candle.Close(1) > candle.Open(2)) && (candle.Open(1) < candle.Close(2))) && (candle.High(1) < candle.Close(2)) && (candle.Low(1) > candle.Open(2)) && (candle.Open(1) != candle.Close(1)); //&& (SombraSup1 >=  SombraInf1)

		boolean PrimeiroCandleBaixa= (candle.Close(0) < candle.Low(2)) && (candle.Close(0) < candle.Open(0));

		boolean HaramiDeBaixa = (TendenciaUltimosCandles==1) && TerceiroCandleAlta && SegundoCandle && PrimeiroCandleBaixa;
