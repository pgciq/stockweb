 ...
 */
public class BearishOnNeck extends Indicator {
	// Padrao Piercing perto do pescoço
...
*/
		...
		boolean SegundoCandleBaixa = (candle.Open(1) > candle.Close(1)) && ( CorpoCandle2 > (SombraSup2+SombraInf2) );

		double rangeMaximo = (((candle.High(1) - candle.Low(1)) / 100) * 18); //Retorna o valor do preço que representa 10%

		boolean PrimeiroCandleAlta = (candle.Close(0) > candle.Open(0)) && ((candle.Close(0) < (candle.Low(1) + rangeMaximo)) && (candle.Close(0) > (candle.Low(1) - rangeMaximo)) ) &&  ( CorpoCandle1 > (SombraSup1+SombraInf1) );

		boolean BearishOnNeck = (TendenciaUltimosCandles==0) && SegundoCandleBaixa && PrimeiroCandleAlta;
