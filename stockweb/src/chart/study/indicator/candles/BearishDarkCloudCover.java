...
public class BearishDarkCloudCover extends Indicator {
	...
	public double calculate() {
		...
		boolean SegundoCandleAlta = (candle.Open(1) < candle.Close(1)) && ( CorpoCandle2 > (SombraSup2+SombraInf2) );

		boolean PrimeiroCandleBaixa = ( CorpoCandle1 > (SombraSup1+SombraInf1) ) && ((candle.Open(0) > candle.Close(1)) && (candle.Close(0) > candle.Open(1))) && (candle.Open(0) > candle.Close(0)) && (candle.Close(0) < candle.Close(1) - range50percCandle1); 

		boolean NuvensNegras = (TendenciaUltimosCandles==1) && SegundoCandleAlta && PrimeiroCandleBaixa;
