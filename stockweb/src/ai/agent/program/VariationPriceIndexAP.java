...
public class VariationPriceIndexAP extends AgentProgram {
	...
	public String getListVariationPriceIndex() {
		...
		for (int i = 5; i < history.getSize(); i++) {
			...
			String differMaxMin = String.valueOf(
					df.format(vpi.getDifferMaxMin())).replace(",", ".");
			String mediaMax = String.valueOf(df.format(vpi.getMediaMax()))
					.replace(",", ".");
			String mediaMin = String.valueOf(df.format(vpi.getMediaMin()))
					.replace(",", ".");

			priceBar = history.getPriceBar(i - 5);
			result += token + "{date:" + priceBar.getDate() + "," + " dMM:"
					+ differMaxMin + "," + " mMax:" + mediaMax + "," + " mMin:"
					+ mediaMin + "}";
			token = "|";
	...
	public String getVariationPriceIndex() {
		...
		String differMaxMin = String.valueOf(df.format(vpi.getDifferMaxMin()))
				.replace(",", ".");
		...
		result = "{type:variationpriceindex,"
				+
				// " date:"+priceBar.getDate()+"," + FIXME NullPointerException
				" dMM:" + differMaxMin + "," + " mMax:" + mediaMax + ","
				+ " mMin:" + mediaMin + "}";
