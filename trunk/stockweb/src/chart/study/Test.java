...
*/
	...
	private void saveObject(String line){
		...
		if(ibov.getCodneg().trim().equals("VALE5")){  
			//System.out.println(ibov.getCodneg() + " - ult = " + ibov.getPreult() + " - abert = " + ibov.getPreabe() + " - data = " + ibov.getDataPregao());			
			priceBar = new PriceBar(date, parseDouble(open), parseDouble(high), parseDouble(low), parseDouble(close), volume);
			history.addHistoricalPriceBar(priceBar);
		}
	}
	...
	private double parseDouble(double price){
		...
		String tmp = value.substring(0, value.length()-4) + "." + value.substring(value.length()-4, value.indexOf("."));
