...
public class VariationVolume extends Indicator {
    ...
    public double calculate() {
        ...
        differMaxMin = Math.abs(((candle.High(firstBar) / candle.Low(firstBar))*100)-100);
        differOpenClose = Math.abs(((candle.Close(firstBar) / candle.Close(firstBar+1))*100)-100);
        ...
        for (int x = firstBar; x <= lastBar; x++) {
        	...
        	mediaMaxMin = mediaMaxMin + Math.abs(((candle.High(x+1) / candle.Low(x+1))*100)-100);
        }
...
/*        for(int x=1; x<ncandles; x++){
        	mediaMaxMin = mediaMaxMin + Math.abs(((candle.High(x) / candle.Low(x))*100)-100);
        }
*/
