...
*/
       ...
       double rangeMaximo = (((candle.Open(1) - candle.Close(1)) / 100) * 20); //Retorna o valor do preço que representa 10%

       boolean PrimeiroCandleAlta= (candle.Close(0) > candle.Open(0)) && ((candle.Close(0) < (candle.Close(1) + rangeMaximo)) && (candle.Close(0) > (candle.Close(1) - rangeMaximo)) ) &&  ( CorpoCandle1 > (SombraSup1+SombraInf1) );

       boolean BearishInNeck = (TendenciaUltimosCandles==0) && SegundoCandleBaixa && PrimeiroCandleAlta;
