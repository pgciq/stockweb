...
public class BearishBeltHold extends Indicator {
	...
	public double calculate() {
		...
		boolean LarguraCandle = (CorpoCandle1 > CorpoCandle2*1.05) && (CorpoCandle1> CorpoCandle3*1.05) && (CorpoCandle1> CorpoCandle4*1.05) && (SombraInf1 > SombraSup1); // && (CorpoCandle1> CorpoCandle5)

		boolean PrimeiroCandleBaixa = (candle.Open(0) > candle.High(1)) && (CorpoCandle1 > (SombraSup1+SombraInf1)) && (candle.Close(0) < candle.Low(0) +range20percCandle2) && ( ((candle.Close(0) > candle.Open(1)) && (candle.Close(0) > candle.Open(1) + range10percCandle2) && (candle.Close(1) > candle.Open(1)) ) || ((candle.Close(0) > candle.Close(1)) && (candle.Close(0) > candle.Close(1) + range10percCandle2) && (candle.Close(1) < candle.Open(1))) ) && (candle.Close(0) < candle.High(1));

		boolean BearishBeltHold = PrimeiroCandleBaixa && LarguraCandle && (TendenciaCandles == 1);
