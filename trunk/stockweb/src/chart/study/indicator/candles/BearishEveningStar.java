...
public class BearishEveningStar extends Indicator {
	...
	public double calculate() {
		...
		boolean SegundoCandleGAP = ((candle.Open(1) > candle.Close(2)) && (candle.Close(1) > candle.Close(2))) && ((candle.Open(1) > candle.Open(0)) && (candle.Close(1) > candle.Open(0))) && (candle.Low(1) > candle.High(2)) && (CorpoCandle2 < CorpoCandle3) && (CorpoCandle2 < CorpoCandle1); //((CorpoCandle2*2) < CorpoCandle3) 
		boolean PrimeiroCandleBaixa = (candle.Open(0) > candle.Close(0)) && (candle.Open(0) >= candle.Open(2)) && (candle.Open(0) > candle.Close(2)) && (CorpoCandle1 > SombraSup1+SombraInf1); //AND ( CorpoCandle1 > (SombraSup1+SombraInf1) ) //AND (Close >= Open(2))
		boolean EstrelaDaNoite = TerceiroCandleAlta && SegundoCandleGAP && PrimeiroCandleBaixa;
		...
		if (EstrelaDaNoite) {
			...
			if ((candle.Close(2) < candle.Open(0)) && (candle.Close(2) > candle.Close(0))) 
				result = -5.5;
		}
