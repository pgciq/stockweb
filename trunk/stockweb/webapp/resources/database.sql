...
INSERT INTO `scripts` (`id`,`script`,`name`,`descr`,`param`,`settingchart`) VALUES 
 ...
 (25,'function applyScript(candle, json, lastbar, script, input) {\n    // $getCandleSize(candle, tipo, sombra)\n    // TIPO = Corpo (1) , Sombra(2) ou Inteiro Candle(3) do Candle\n    // Sombra = Superior (1) ou Inferior (2)\n    var result = 0;\n\n    var tipo = eval(input.get(\'tipo\'));\n    var sombra = eval(input.get(\'sombra\'));\n   \n//    println(\"tipo: \" + tipo);\n//    println(\"sombra: \" + sombra);\n\n    var SinalCandle = (candle.Close(lastbar) > candle.Open(lastbar));\n    if (tipo == 1) {\n        if (SinalCandle) {\n            result = Math.abs(((candle.Close(lastbar) / candle.Open(lastbar)) * 100) - 100);\n        } else {\n            result = Math.abs(((candle.Open(lastbar) / candle.Close(lastbar)) * 100) - 100);\n        }\n\n    } else if (tipo == 2) {\n\n        if (SinalCandle) {\n            if (sombra == 1) {\n                result = Math.abs(((candle.High(lastbar) / candle.Close(lastbar)) * 100) - 100);\n            } else if (sombra == 2) {\n                result = Math.abs(((candle.Open(lastbar) / candle.Low(lastbar)) * 100) - 100);\n            }\n        } else { if (sombra == 1) {\n                result = Math.abs(((candle.High(lastbar) / candle.Open(lastbar)) * 100) - 100);\n            } else if (sombra == 2) {\n                result = Math.abs(((candle.Close(lastbar) / candle.Low(lastbar)) * 100) - 100);\n            }\n        }\n\n        if (tipo == 3) {\n            result = Math.abs(((candle.High(lastbar) / candle.Low(lastbar)) * 100) - 100);\n        }\n        \n        \n    }\n    json.put(\"return\", result);\n    return json;\n}\n','$getCandleSize','$getCandleSize(position, tipo, sombra)','return',''),
 (26,'function applyScript(candle, json, lastbar, script, input) {\n    var result = 0;\n\n    //=================================================================================\n    // Bullish Hanging Man\n\n    input.put(\"tipo\", \"1\");\n    input.put(\"sombra\", \"0\");\n    var CorpoCandle1 = script.call(\"$getCandleSize\", lastbar, input); //[0 ,1 ,0]\n    println(\"CorpoCandle1: \" + CorpoCandle1);\n\n    input.put(\"tipo\", \"2\");\n    input.put(\"sombra\", \"1\");\n    var SombraSup1 = script.call(\"$getCandleSize\", lastbar, input); //[0 ,2 ,1]\n    println(\"SombraSup1 : \" + SombraSup1);\n\n    input.put(\"tipo\", \"2\");\n    input.put(\"sombra\", \"2\");\n    var SombraInf1 = script.call(\"$getCandleSize\", lastbar, input); //[0 ,2 ,2]\n    println(\"SombraInf1 : \" + SombraInf1 );\n\n    input.put(\"begin\", lastbar);\n    input.put(\"end\", input.get(\"candles\"));\n    var TendenciaUltimosCandles = script.call(\"$getTendencia\", lastbar, input); //[0,candles]\n    println(\">>>> TendenciaUltimosCandles : \" + TendenciaUltimosCandles);\n    println(\"date: \" + candle.Date(lastbar));\n\n    var PrimeiroCandleBaixa = (candle.Open(lastbar + 1) > candle.Close(lastbar + 1)) && (CorpoCandle1 > (SombraSup1 + SombraInf1) * 1.2);\n\n    var PrimeiroCandleAlta = (candle.Open(lastbar + 1) < candle.Close(lastbar + 1)) && (CorpoCandle1 > (SombraSup1 + SombraInf1) * 1.2);\n\n    var CandleLongDownTrend = (TendenciaUltimosCandles == 0) && (PrimeiroCandleBaixa);\n\n    var CandleLongUpTrend = (TendenciaUltimosCandles == 1) && (PrimeiroCandleAlta);\n\n\n    if (CandleLongDownTrend) {\n        result = -1;\n    }\n\n    if (CandleLongUpTrend) {\n        result = 1;\n    }\n\n    json.put(\"return\", result);\n\n    return json;\n}','$isCandleLongTrend','$isCandleLongTrend(candles)','return','');
